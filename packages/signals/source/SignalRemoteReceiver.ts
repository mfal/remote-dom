...
import {getNodePosition} from '@remote-dom/core/utils';
  ...
  constructor({retain, release}: RemoteReceiverOptions = {}) {
    ...
    const baseConnection = createRemoteConnection({
      ...
      insertChild: (id, child, parentRemoteId) => {
        ...
        const position = getNodePosition(newChildren, parentRemoteId) + 1;
      ...
      removeChild: (id, remoteId) => {
        ...
        const newChildren = [...parent.children.peek()];
        const position = getNodePosition(newChildren, remoteId);
